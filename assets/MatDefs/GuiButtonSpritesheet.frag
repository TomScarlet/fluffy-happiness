...
void main() {
    ...
    if (buttonCoord.x <= m_BorderWidth) {
        ...
        // Right border
        drawTexCoord.x = 1 - (1 - texCoord.x) * m_ButtonSize.x / m_ImageSize.x;
    } else {
        // Middle section
        drawTexCoord.x = (mod(texCoord.x * m_ButtonSize.x - m_BorderWidth, m_ImageSize.x - 2 * m_BorderWidth) + m_BorderWidth) / m_ImageSize.x;
    }
    if (buttonCoord.y <= m_BorderWidth) {
        ...
        // Top border
        drawTexCoord.y = 1 - (1 - texCoord.y) * m_ButtonSize.y / m_ImageSize.y;
    } else {
        // Middle section
        drawTexCoord.y = (mod(texCoord.y * m_ButtonSize.y - m_BorderWidth, m_ImageSize.y - 2 * m_BorderWidth) + m_BorderWidth) / m_ImageSize.y;
    }
